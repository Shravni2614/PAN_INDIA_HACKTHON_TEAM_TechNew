...
# Conversation States
# ---------------------------
MENU, MARKET, WEATHER, HARVEST, LOCATION_WEATHER, MARKET_STATE, LOCATION_MENU = range(7)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• Find your exact location details\n"
...
async def location_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show location-based services menu"""
    keyboard = [
        ["📍 My Location Info"],
        ["🌤 Weather at My Location"],
        ["🌾 Harvest Advice for My Area"],
        ["🏪 Nearest Mandi"],
    ...
    await update.message.reply_text(
        "📍 *Location-Based Services*\n\n"
        "Select an option:\n\n"
        "• *My Location Info* - Get address, PIN code, local time\n"
        "• *Weather at My Location* - Forecast for your exact location\n"
        "• *Harvest Advice for My Area* - Customized farming advice\n"
        "• *Nearest Mandi* - Find closest agricultural market\n"
        "• *My Agricultural Zone* - Know your farming zone and major crops",
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )
...
async def handle_location_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle location menu selections"""
    text = update.message.text
    
    # Store the action user wants to perform
    action_map = {
        "📍 My Location Info": "info",
        "🌤 Weather at My Location": "weather",
        "🌾 Harvest Advice for My Area": "harvest",
        "🏪 Nearest Mandi": "mandi",
        "🌱 My Agricultural Zone": "zone"
    }
    
    if text in action_map:
        context.user_data['last_location_action'] = action_map[text]
        await update.message.reply_text(
            f"📍 Please share your location using the attachment button 📎\n\n"
            f"Tap the 📎 icon, then select 'Location' to share your current location.",
            reply_markup=ReplyKeyboardMarkup([["🔙 Back to Menu"]], resize_keyboard=True)
        )
        return LOCATION_WEATHER
...
# ---------------------------
async def enhanced_location_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle location with enhanced features"""
    ...
    if not update.message.location:
        await update.message.reply_text(
            ...
            "1. Tap the 📎 icon near the text input\n"
    ...
    # Show loading
    loading_msg = await update.message.reply_text("📍 Processing your location...")
    
    try:
        # Get detailed location info
        location_info = location_service.get_location_from_coords(lat, lon)
        
        # Get local time
        local_time = location_service.get_local_time(lat, lon)
        
        # Get nearest mandi
        nearest_mandi = location_service.get_nearest_mandi(lat, lon)
        
        # Get agricultural zone
        agri_zone = location_service.get_agricultural_zone(lat, lon)
        
        await loading_msg.delete()
        
        # Determine what the user wanted based on previous menu selection
        last_action = context.user_data.get('last_location_action', 'info')
        
        if last_action == "info":
            # Show comprehensive location info
            message = (
                f"📍 *Your Location Information*\n\n"
                f"🌍 *Coordinates:*\n"
                f"Latitude: `{lat:.4f}`\n"
                f"Longitude: `{lon:.4f}`\n\n"
                ...
                f"District: {location_info['district']}\n"
                f"City/Village: {location_info['city']}\n"
                f"PIN Code: {location_info['pin_code']}\n\n"
                
                f"⏰ *Local Time:*\n"
                f"{local_time.strftime('%I:%M %p, %d %B %Y')}\n\n"
                
                f"🌱 *Agricultural Zone:* {agri_zone}\n\n"
                
                f"🏪 *Nearest Mandi:*\n"
                f"{nearest_mandi['name']}\n"
                f"📏 Distance: {nearest_mandi['distance_km']} km\n"
                f"📍 {nearest_mandi['state']}\n\n"
                
                f"_Data source: OpenStreetMap, data.gov.in_"
            )
            
            await update.message.reply_text(message, parse_mode='Markdown')
            
        elif last_action == "weather":
            # Call weather function with coordinates
            return await weather_by_location(update, context, lat, lon, location_info)
            
        elif last_action == "harvest":
            # Call harvest advice with coordinates
            return await harvest_by_location(update, context, lat, lon, location_info)
            
        elif last_action == "mandi":
            # Show nearest mandi info
            message = (
                f"🏪 *Nearest Agricultural Market*\n\n"
                f"📍 *Mandi:* {nearest_mandi['name']}\n"
                f"📏 *Distance:* {nearest_mandi['distance_km']} km\n"
                f"🗺️ *State:* {nearest_mandi['state']}\n\n"
                
                f"💡 *Tip:* You can check market prices for this mandi\n"
                f"by selecting 'Market Prices' from main menu.\n\n"
                
                f"📍 *Location Coordinates:*\n"
                f"Lat: {nearest_mandi['lat']}, Lon: {nearest_mandi['lon']}"
            )
            await update.message.reply_text(message, parse_mode='Markdown')
            
        elif last_action == "zone":
            # Show agricultural zone info
            zone_info = {
                "North": "Wheat, Rice, Sugarcane, Cotton, Maize",
                "South": "Rice, Coffee, Tea, Spices, Coconut, Banana",
                "East": "Rice, Jute, Tea, Vegetables, Mustard",
                "West": "Cotton, Groundnut, Onion, Bajra, Soybean",
                "Central": "Soybean, Wheat, Gram, Pulses, Sunflower"
            }.get(agri_zone, "Mixed crops depending on local conditions")
            
            message = (
                f"🌱 *Your Agricultural Zone: {agri_zone}*\n\n"
                f"*Major Crops in this zone:*\n"
                f"{zone_info}\n\n"
                f"*Recommended Actions:*\n"
                f"• 🌾 Check market prices for these crops regularly\n"
                f"• 🌤 Monitor weather forecast before sowing\n"
                f"• 📅 Plan harvest based on seasonal patterns\n"
                f"• 🏪 Find nearest mandi for better price discovery"
            )
            await update.message.reply_text(message, parse_mode='Markdown')
        
        # Clear the action
        context.user_data.pop('last_location_action', None)
        ...
        await loading_msg.delete()
        await update.message.reply_text(
            "❌ Error processing your location.\n"
            "Please try again or use city name instead."
        )
        return MENU

# ---------------------------
# Weather by Location (Enhanced)
# ---------------------------
async def weather_by_location(update: Update, context: ContextTypes.DEFAULT_TYPE, lat=None, lon=None, location_info=None):
    """Fetch weather forecast using location coordinates"""
    
    if lat is None or lon is None:
        if 'last_location' not in context.user_data:
    ...
    # Show loading
    loading_msg = await update.message.reply_text(f"🌤 Getting weather for {city}, {state}...")
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"🕐 *Local Time:* {location_service.get_local_time(lat, lon).strftime('%I:%M %p')}",
            parse_mode='Markdown'
        )
...
# Harvest by Location
# ---------------------------
async def harvest_by_location(update: Update, context: ContextTypes.DEFAULT_TYPE, lat=None, lon=None, location_info=None):
    """Provide harvest advice based on location coordinates"""
    ...
    try:
        ...
        for entry in data["list"][:24]:  # 3 days
            rain = entry.get("rain", {}).get("3h", 0)
            total_rain += rain
            if rain > 0:
                rain_days += 1
        ...
        # Get agricultural zone for crop recommendations
        agri_zone = location_service.get_agricultural_zone(lat, lon)
        
        # Zone-specific crop recommendations
        zone_crops = {
            "North": ["Wheat", "Rice", "Sugarcane", "Cotton", "Maize"],
            "South": ["Rice", "Coffee", "Tea", "Coconut", "Banana", "Spices"],
            "East": ["Rice", "Jute", "Tea", "Vegetables", "Mustard"],
            "West": ["Cotton", "Groundnut", "Onion", "Bajra", "Soybean"],
            "Central": ["Soybean", "Wheat", "Gram", "Pulses", "Sunflower"]
        }
        
        recommended_crops = zone_crops.get(agri_zone, ["Consult local agricultural office"])
        
        # Generate advice
        location_display = f"{city}, {state}" if state else city
        message = f"🌾 *Harvest Advice for {location_display}*\n\n"
        message += f"📍 Zone: {agri_zone}\n\n"
        message += f"📊 *Weather Analysis (Next 3 days)*\n"
        message += f"• Expected Rain: {total_rain:.1f}mm\n"
        message += f"• Rainy periods: {rain_days}/8\n"
        message += f"• Avg Temperature: {avg_temp:.1f}°C\n"
        message += f"• Avg Humidity: {avg_humidity:.0f}%\n\n"
        ...
        if total_rain < 2:
            ...
            message += "• Delay harvesting for 4-5 days\n"
            message += "• Monitor field for waterlogging\n"
            message += "• Check crops for diseases\n\n"
        
        # Crop recommendations for this zone
        message += f"🌱 *Recommended Crops for {agri_zone} Zone:*\n"
        for crop in recommended_crops[:5]:
            message += f"• {crop}\n"
        
        # Seasonal advice
        current_month = location_service.get_local_time(lat, lon).month
        if 3 <= current_month <= 6:
            message += "\n🌸 *Summer Season:* Focus on heat-resistant varieties"
        elif 7 <= current_month <= 10:
            message += "\n🌧️ *Monsoon Season:* Ensure proper drainage"
        else:
            message += "\n❄️ *Winter Season:* Protect from frost"
        
        await update.message.reply_text(message, parse_mode='Markdown')
        
    except Exception as e:
        logger.error(f"Error in harvest by location: {e}")
        await update.message.reply_text("❌ Error generating harvest advice.")
...
async def weather(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Ask for next action
        keyboard = [["🌤 Another City"], ["📍 Location Services"], ["🔙 Back to Menu"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text(
            "What would you like to do next?",
            reply_markup=reply_markup
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle special options from market price flow
    if text == "📋 Show All Crops" or text == "📊 View All Crops":
        return await show_all_crops(update, context)
    ...
    if text == "📈 Market Prices":
        ...
        await update.message.reply_text(
            "Enter city name (e.g., Mumbai, Pune, Delhi):\n\n"
            "Or use 📍 Location Services for automatic detection."
        )
        return WEATHER

    elif text == "📍 Location Services":
        return await location_menu(update, context)

    elif text == "🌾 Harvest Advice":
        await update.message.reply_text(
            "Enter your city name for harvest advice:\n\n"
            "Or use 📍 Location Services for advice based on your exact location."
        )
        return HARVEST
...
async def market_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show available crops for this state
    available_crops = market_data.get_all_crops()
    
    await update.message.reply_text(
        f"Selected state: *{text}*\n\n"
        f"Enter crop name (e.g., Onion, Potato, Tomato, Wheat, Rice):\n\n"
        f"*Available in database:* {', '.join(available_crops[:7])}...",
        parse_mode='Markdown'
    )
    return MARKET
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "*Features:*\n"
        "📈 *Market Prices* - Get crop prices by state (hybrid: local + API)\n"
        "🌤 *Weather* - Forecast by city name or location\n"
        "📍 *Location Services* - Get info about your current location\n"
        "🌾 *Harvest Advice* - Best time to harvest based on weather\n\n"
        "*How to Share Location:*\n"
        "1. Tap the 📎 icon\n"
        "2. Select 'Location'\n"
    ...
    try:
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                WEATHER: [MessageHandler(filters.TEXT & ~filters.COMMAND, weather)],
                LOCATION_WEATHER: [
                    MessageHandler(filters.LOCATION, enhanced_location_handler),
                    MessageHandler(filters.TEXT & ~filters.COMMAND, enhanced_location_handler)
                ],
        ...
        # Start the bot
        print("✅ Smart Krishi AI Bot is running... Press Ctrl+C to stop.")
        print("📱 Test your bot on Telegram now!")
        print("📍 Location Services enabled - Users can share their location!")
        app.run_polling()
